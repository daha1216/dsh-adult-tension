...
| 内容 | 数量 | 示例 |
| --- | --- | --- |
| 时代与地点 | 时代 27 · 地点 98 | 架空王朝、明治东京、秋叶原架空街区 |
| 张力引擎 | 57 条 | 秘密暴露倒计时、债务压力、双重马甲互掉现场 |
| 压力来源 | 45 条 | 家人病倒、官司上门、证件卡壳 |
| 角色身份 | NPC 17 族 90 身份 · 玩家化身 33 种 | 权力治理、家族继承、服务与手艺 |
| 角色处境 | 60 条 | 秘密将破、债务压身、晋升窗口 |
| 人物性格与反差 | 反差轴 24 种 | 外冷内热、端庄放浪、毒舌心软 |
| 关系与权力 | 4 种 | 玩家上位、NPC 上位、平等或动态切换 |
| 语言风格 | 表层风味 131 · 口癖语感 51 | 敬语、口癖、拟声词等 |
| 场景动作 | 交易摊牌 22 · 非交易靠近 39 | 递外套、承认紧张、当面摊牌 |
| 中期转折池 | 7 类 × 13~16（共 101 条） | 信息类、资源类、时限类等 |
| 配角功能 | 15 类 | 盟友、竞争者、误导者、催化剂 |

此外还有决策三轴、外观轴、称谓（27 条）、美学基调（32 种，写实基调自动收口外观与口癖）、命名库（通用 30 姓 · 40 名 + 27 个时代专属名池）等更细的生成素材。数量为 `scripts/data/*.yaml` 当前实测值；想扩充素材或调整文案见 [`references/加内容.md`](./references/加内容.md)，改完可用 `python scripts/check_content.py` 做一键内容体检。
...
维护者向的工具：`commit_turn.py` 是回合提交器（时钟、场景、事件、许可与提交前校验一次落地）；`check_content.py` 对全部素材数据做内容体检（当前 434 项检查）。扩充素材的方法见 [`references/加内容.md`](./references/加内容.md)。
