...
**内置数百项素材的世界库，NPC 会记得你**——从时代地点、角色身份到张力与压力，随机开局组合出几乎不重复的完整新故事；角色有自己的立场与底线，会犹豫、拒绝，也可能在你离开后自行行动。
...
| 内容 | 数量 | 示例 |
| --- | --- | --- |
| 时代与地点 | 时代 10 · 地点 23 | 古代宫廷、近未来都市、夜场会所 |
| 张力引擎 | 21 条 | 秘密暴露、债务压力、嫉妒与占有 |
| 压力来源 | 15 条 | 家人病倒、官司上门、证件卡壳 |
| 角色身份 | NPC 12 类 · 玩家化身 8 类 | 权力治理、家族继承、服务与手艺 |
| 角色处境 | 16 条 | 秘密将破、债务压身、晋升窗口 |
| 人物性格与反差 | 反差轴 8 种 | 外冷内热、端庄放浪、毒舌心软 |
| 关系与权力 | 4 种 | 玩家上位、NPC 上位、平等或动态切换 |
| 语言风格 | 表层风味 73 · 口癖语感 12 | 敬语、口癖、拟声词等 |
| 场景动作 | 交易摊牌 8 · 非交易靠近 12 | 递外套、承认紧张、摊牌谈判 |
| 中期转折池 | 7 类 × 3 | 信息类、资源类、时限类等 |
| 配角功能 | 13 类 | 盟友、竞争者、误导者、催化剂 |

此外还有决策三轴、外观轴、称谓（14 条）、命名库（姓氏 30 · 名 40）、美学基调等更细的生成素材。数量为 `scripts/data/*.yaml` 当前实测值；想扩充素材或调整文案见 [`references/加内容.md`](./references/加内容.md)，改完可用 `python scripts/check_content.py` 做一键内容体检。
...
维护者向的工具：`commit_turn.py` 是回合提交器（时钟、场景、事件、许可与提交前校验一次落地）；`check_content.py` 对全部素材数据做内容体检。扩充素材的方法见 [`references/加内容.md`](./references/加内容.md)。
