...
* **完全交给系统随机**：
  > *「开局」*  
  > （系统会询问你想要紧迫刺激的「压力模式」，还是慢节奏生活流的「日常模式」）
* **带着你的想法直接开**（题材、身份、关系随心定）：
  > *「开局，想玩盛唐长安，大理寺暗查案卷的故事」*  
  > *「开局，民国上海滩，我是个落魄琴师，对面是舞厅大班」*

**开始前想加点定向控制？** 支持多种极简人话输入：

| 想要的效果 | 极简输入示例 | 背后效果 |
| :--- | :--- | :--- |
| **定死困境** | `开局，旧事上门，其他随机` | 锁定核心危机，配角与地点随机生成 |
| **只用官方库** | `开局，只选自带世界` | 仅从系统内置的 52 个世界框架与精选素材池中抽取 |
| **完全自创新世界** | `开局，自拟全新世界` | 突破内置素材库，由 AI 自由发挥捏一个新世界 |
| **复现相同开局** | `开局，重开 42 号` | 按相同编号百分之百还原该局世界与人物配置 |

---

### 2. 推进剧情

新局生成时，依次包含 `世界观`、`人物`、`正文` 三个部分（前两项各 1-2 句交代背景，仅新局呈现，载入与续玩不重复三个标题）。正文会自然停在你能接手的动作前。

**无需任何特定格式**，直接描述角色的动作或台词即可：
...
故事开始后，用这些自然指令推进和管理进度：

| 命令 | 什么时候用 |
| --- | --- |
| `继续`（`c`） | 剧情停在可接续处时，向前推一个节拍 |
| `快进到明天早上` | 跳过一段时间，场景外的离屏事件会被追算、世界自己长 |
| `状态` | 查看看板：所在地点、在场人物、当前压力、所处情境与可接续动作 |
| `存档`（`qs`） | 写入当前存档槽；成功只回一句「已保存到「名称」·第 N 回合」，不刷屏 |
| `保存 X` / `载入 X` | 另存为命名存档 X 或从原节点继续，支持开辟多条独立走向 |
| `列出存档` | 显示所有命名存档的名称、回合与一句话处境 |
| `导出存档` / `载入存档` | 将当前局面导出为 YAML 文本备份，发回即可原地续玩 |
| `撤销刚才的 X` | 只撤销最近一个事件，不推进回合 |

> 提示：高级功能随用随说：`帮助` 列出全部命令，另有贴视角、语态切换、冻结推演等，触发词详见 [`commands.yaml`](./commands.yaml)。一个会话默认使用自己的专属存档；保存时若存档已被其他窗口修改，会提示你读取最新版本、另存为其他名称或取消，不会静默覆盖。
