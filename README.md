...
**多窗口，各玩各的**——每个会话独立存档槽，互不覆盖；想开分支或明确协作时也能随时切换。
...
| `查看存档槽` | 列出可用存档槽、分支与版本。多个窗口同时游玩时，可避免互相覆盖 |
| `切换存档槽 X` | 绑定存档槽 X，继续或接管该槽的主线 |
| `新建存档槽 X` / `新建分支 X` | 从当前已提交状态开一条独立走向，各自续玩不互相覆盖 |
| `打开共享存档 X` | 多个窗口明确协作推进同一条主线时才使用；写入由会话租约保护 |
| `暂停` / `安全词` | 立刻停止亲密升级，任何时候都可以 |

> 一个会话绑定一个存档槽；默认新会话使用独立槽位，避免多个窗口悄悄覆盖同一份剧情。只有明确选择同一槽位时才共享写入，并受版本校验保护。
...
多窗口存档由 `manage_saves.py` 管理：每个槽位包含 `state.yaml`（v3 叙事状态）与 `manifest.yaml`（版本、哈希、租约）。保存必须携带载入时的 `--expected-revision`，版本不一致会被拒绝且不自动合并；需要分支时可用 `branch`。共享槽需先 `mode <slot> shared` 再 `acquire` 租约。
