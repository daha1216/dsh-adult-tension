...
| 内容 | 数量 | 示例 |
| --- | --- | --- |
| 时代与地点 | 时代 34 · 地点 116 | 盛唐长安百鬼夜行、一九二八芝加哥禁酒期、文久幕末京都风云 |
| 张力引擎 | 74 条 | 天枢锁妖大阵灵气逆流、黑帮教父暗账审计之夜、同人音声双耳麦克风实景录音 |
| 压力来源 | 66 条 | 宵禁六百响更鼓将毕、同人展压盘死线仅剩三小时、经纪人在录音室门外敲门查岗 |
| 角色身份 | NPC 17 族 90 身份 · 玩家化身 33 种 | 权力治理、家族继承、服务与手艺 |
| 角色处境 | 69 条 | 秘密将破、妖狐内丹子时反噬、洗刀水色变红的瞬间、录音红灯亮起时的实景耳语夜 |
| 人物性格与反差 | 反差轴 26 种 | 外冷内热、端庄放浪、禁欲破戒、严苛沦陷 |
| 关系与权力 | 4 种 | 玩家上位、NPC 上位、平等或动态切换 |
| 语言风格 | 表层风味 131 · 口癖语感 51 | 敬语、口癖、拟声词等 |
| 场景动作 | 交易摊牌 22 · 非交易靠近 50 | 指尖朱砂点眉心镇妖、递白布替她拭刀、在防喷罩前替她理顺耳机线 |
| 中期转折池 | 7 类 × 13~16（共 101 条） | 信息类、资源类、时限类等 |
| 配角功能 | 15 类 | 盟友、竞争者、误导者、催化剂 |

此外还有决策三轴、外观轴、称谓（27 条）、美学基调（46 种，写实基调自动收口外观与口癖）、命名库（通用 30 姓 · 40 名 + 34 个时代专属名池）等更细的生成素材。数量为 `scripts/data/*.yaml` 当前实测值；想扩充素材或调整文案见 [`references/加内容.md`](./references/加内容.md)，改完可用 `python scripts/check_content.py` 做一键内容体检。
...
维护者向的工具：`commit_turn.py` 是回合提交器（时钟、场景、事件、许可与提交前校验一次落地）；`check_content.py` 对全部素材数据做内容体检（当前 534 项检查）。扩充素材的方法见 [`references/加内容.md`](./references/加内容.md)。
