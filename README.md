...
**开始前想加点控制？** 开局由一套「随机引擎」生成，你可以用几个开关决定它多听话：

| 你想干嘛 | 输入 | 效果 |
| --- | --- | --- |
| 复现同一个开局 | `种子 42` | 固定随机种子：同一个数字永远生成同一套开局，方便复盘、对比或分享 |
| 先定人设或题材 | `预锁 身份=帝国女帝` | 只锁定想锁的维度（身份、处境、引擎、题材…），其余照常随机 |
| 内容严格出自内置素材 | `强制表内` | 所有要素都从内置素材池抽取，不越出表外 |
| 放开来自由发挥 | `表外全随机` | 允许自拟与更“野”的开局，跳出素材表的限制 |
